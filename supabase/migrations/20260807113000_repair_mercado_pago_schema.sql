-- thegestor | reparo idempotente do schema Mercado Pago
-- Pode ser executado mesmo se a migration 20260807110000 falhou ou foi parcialmente aplicada.
-- Sem BEGIN global de propósito: evita que um erro posterior reverta as colunas básicas.

alter table public.pagamentos add column if not exists provider_order_id text;
alter table public.pagamentos add column if not exists pix_ticket_url text;
alter table public.pagamentos add column if not exists pix_qr_code text;
alter table public.pagamentos add column if not exists pix_qr_code_base64 text;
alter table public.pagamentos add column if not exists expira_em timestamptz;
alter table public.pagamentos add column if not exists idempotency_key text;
...
-- RPC usada pelo admin logado ao gerar o Pix.
...
begin
  ...
  values (
    p_empresa_id, 'mercado_pago', 'principal', 'conectada',
    jsonb_build_object('api', 'orders', 'payment_method', 'pix'), now()
  )
...
-- RPC usada somente pelo backend após consultar a Order no Mercado Pago.
...
-- Força o PostgREST/Supabase a recarregar o schema após novas colunas/funções.
